...
|---------|--------------|-----------|
| Windows | [setup.exe](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-standard_4.2.0_x64-setup.exe) | [setup.exe](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-light_4.2.0_x64-setup.exe) |
| macOS | [dmg 映像](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-standard_4.2.0_aarch64.dmg) | [dmg 映像](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-light_4.2.0_aarch64.dmg) |
| Linux | [deb 包](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-standard_4.2.0_amd64.deb) | [deb 包](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame-light_4.2.0_amd64.deb) |

- **安卓预览版**：[apk](https://github.com/kabegame/kabegame/releases/download/v4.2.0/Kabegame_4.2.0_android-preview.apk)（同一发布页）。
...
```bash
# 开发模式（带 watch，热重载）
bun dev -c kabegame              # 启动主应用（端口 1420）
bun dev -c kabegame --mode local # 使用 local 模式（无商店版本，预打包全部插件）

# 启动模式（无 watch，直接运行）
bun start -c kabegame-cli            # 启动 CLI 工具

# 构建生产版本
bun b                    # 构建全部组件（kabegame + kabegame-cli）
bun b -c kabegame            # 构建主应用
bun b -c kabegame-cli             # 构建 CLI 工具

# 检查（不产出构建产物）
bun check -c kabegame                # 依次检查 vue 与 cargo
bun check -c kabegame --skip cargo   # 仅检查 vue
...
- `--data`：数据目录模式
  ...
  - 示例：`bun dev -c kabegame --data prod` 可在开发时访问已安装版本的数据
...
- kabegame 应用 `bun dev -c kabegame` 会通过 nx 执行 `crawler-plugins:package-to-dev-data`，将打包好的 `.kgpg` 输出到仓库根目录的 `data/plugins-directory` 供本地调试；正式安装包不再内置商店插件，用户从 GitHub 商店源下载安装
...
Android 开发需使用 **`bun dev -c kabegame --mode android`**（不能省略 `--mode android`，否则会跑桌面版）。若连接了多台设备（真机 + 模拟器），Tauri 可能选错设备，可指定设备 ID：
...
bun dev -c kabegame --mode android -- <设备ID>
```
