...
- Use `kabegame_core::media::image_type::*` (e.g. `is_image_by_path`, `supported_image_extensions`) instead of hardcoding extensions or MIME values in Rust. Frontend uses the `get_supported_image_types` Tauri command; its `mimeByExt` values remain standard MIME.
