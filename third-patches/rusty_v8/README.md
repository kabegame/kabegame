...
**source** for the Android self-built `librusty_v8` archive, produced by `bun run build:v8`
...
it links the archive from `bun run build:v8` via `RUSTY_V8_ARCHIVE`. So the crates.io `v8` stays for
...
`bun run patch rusty_v8` is a no-op here: patch-manager only applies to a **clean** tree (and only
...
`bun run build:v8` writes to `bin/android/` (gitignored, reproduced by command, NOT committed):
...
3. `bun run build:v8` — first run fetches nested submodules + NDK and builds from source (~15 GB),
