...
- `0006-drag-source-bindings-linux.patch` — adds the Linux bindings for `CefDragHandler::OnStartDragging` and `CefDragData::GetCustomData`, including the safe wrapper methods. The generated C layout is authoritative: `_cef_drag_handler_t::on_start_dragging` is at offset 80 (size 88), and `_cef_drag_data_t::get_custom_data` is at offset 256 (size 264).
- `0007-drag-source-bindings-windows.patch` — the same drag-source bindings and verified C layout for `x86_64_pc_windows_msvc`.
- `0008-drag-source-bindings-macos.patch` — the same drag-source bindings and verified C layout for both `aarch64_apple_darwin` and `x86_64_apple_darwin`.
