...
- **同一时间只跑一个 driver**：三个 crate 共用根 target/，并行的第二个 cargo 会
  ...
  串行跑。
- **`-c kabegame` 会做 CEF 检查**：注入 `CEF_PATH`（dev/check/test/build 共用 cef-build），
...
| `CEF runtime not found in: .../cef-build` | `-c kabegame` 需要本机 CEF runtime（`deno task build:chromium`），或者改测 kabegame-core/cli |
