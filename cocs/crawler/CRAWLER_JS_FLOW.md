...
```
用户启动插件任务
    -> TaskScheduler::enqueue 在提交时解析插件、校验版本、合并配置并冻结 TaskParams
    ...
    -> worker await completion 后统一 transition、销毁窗口、remove_run
```
...
| 层级 | 文件路径 | 作用 |
|------|----------|------|
| 任务调度 | `src-tauri/kabegame-core/src/crawler/task_scheduler/mod.rs` | `TaskScheduler` 注册表、提交时冻结参数、worker、取消和 completion 收尾 |
| 任务条目 | `src-tauri/kabegame-core/src/crawler/task_scheduler/task.rs` | `Task` / `TaskParams`，保存 progress、headers、page stack、WebView state、completion、CancellationToken |
| WebView 桥 | `src-tauri/kabegame-core/src/crawler/webview.rs` | 仅保留 `CrawlerWebViewHandler` trait、窗口 label 工具和 `TaskCompletion` 导出 |
| 窗口创建与注入 | `src-tauri/kabegame/src/startup.rs` | 从 `Task` 取 crawl.js/config，创建 crawler 窗口并处理 native download 回调 |
| Tauri 命令 | `src-tauri/kabegame/src/commands/crawler.rs` | 按 `crawler-<task_id>` label 找 `Task`，维护 page stack/state、下载、日志、进度、completion |
...
- 普通插件：提交时执行 `resolve_plugin_for_task_request`、`check_min_app_version`、`resolve_crawl_output_dir`、`build_effective_user_config_from_var_defs`，并把结果存入 `TaskParams`。
- `local-import`：`TaskParams.plugin = None`，`TaskParams.config` 直接保存用户配置。
- `TaskParams` 不保存 `plugin_version` / `base_url` 字段；通过 `plugin_version()` / `base_url()` 从 `plugin: Option<Arc<Plugin>>` 派生。
...
- `Task.webview` 保存 completion sender 与 `Kabegame.state()` 的任务级 state。
...
`crawl_exit` / `crawl_error` / 任务取消都通过 `TaskScheduler::complete_webview_task` 通知 worker。取消顺序为：
