...
- **自建产物（官方无 Android 预编译）**：`rusty_v8` 自 v0.102.0（2024 年中）起不再发布 Android 预编译静态库，且 crates.io 的 v8 包内也没有 Android 版 `src_binding_*.rs`——两个产物都缺。因此需一次性自建，放在 **`bin/android/`**（**gitignore、不入库，由 `bun run build:v8` 复现**）：
