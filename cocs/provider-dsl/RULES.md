...
[`schema.json5`](../../src-tauri/kabegame-core/src/providers/dsl/schema.json5) 校验语法；本文档校验语义。
...
resolve 一条路径 `<scheme>://<seg₁>/<seg₂>/.../<segₙ>` 时引擎执行：
...
```json5
"order": [
    { "sql": "images.crawled_at", "order": "asc" },
    { "sql": "images.size", "order": "desc", "prepend": true },
    { "sql": "ai.\"order\"", "order": "asc", "prepend": true, "clear": "all" }
]
```
...
- `revert` 翻转上游所有 `ASC` ↔ `DESC`；`asc` / `desc` 强制将上游所有字段统一为该方向。
- 全局指令不新增字段，只影响已有 entries 渲染时的方向解释；多次声明 last-wins。
- 一般用于 `desc` 这类反向视图节点。
...
Resolve delegate 的 query scope chain 由 runtime 统一折叠；规范性合约见
[`DESIGN_SPEC.md` §8.8](../../src-tauri/pathql-rs/DESIGN_SPEC.md#88-delegate-scope-chain)。
...
| `${global.<key>}` | 任何 TemplateExpr / SqlExpr / `ProviderCall.properties` 值 / `note:` 字段 | host 注入的运行时只读全局值；SQL 中按 bind param 渲染 |
...
| `${global:<prefix>|<selector>}` | 纯字符串模板：list key / note / properties / 对象或数组 meta 字符串 | 先用当前上下文求值 selector，再查 `${prefix}.${selector_value}` 对应的 runtime global，返回字符串 |

`global` 方法用于 host-owned 显示映射，不用于 SQL inline。例：

```json5
"${global:vd_en_US_month|root.name}"
```

若 `root.name = "01"`，引擎读取 runtime global `vd_en_US_month.01`。该形态在
list key 中若读取 `data_var` / `child_var`，该 key 仍归类为动态 key。

### 6.4 约束

- 模板变量 **不可嵌套**（`${${X}.Y}` 非法）
- 命名空间名 / 方法名不可与 §8 保留标识符冲突
- `${ref:<ident>}` 的参数必须是单个 ident
- `${global:<prefix>|<selector>}` 的 `prefix` 是 runtime global key 前缀，`selector` 是不带 `${...}` 外壳的普通模板路径（如 `root.name` / `row.id`）
...
## 13. Host schema registration contract

Host must register every runtime root before resolving paths:
...
- 语法 schema：[../../src-tauri/kabegame-core/src/providers/dsl/schema.json5](../../src-tauri/kabegame-core/src/providers/dsl/schema.json5)
