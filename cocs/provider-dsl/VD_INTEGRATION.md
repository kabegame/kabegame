...
VD 的 PathQL 根为 `images://vd`，由 `images_root_provider.list.vd` 路由到 `vd_root_router`。
之下依次分三层：

```
images://vd/                                  ← images_root_provider.list 入口
images://vd/i18n-<locale>/                    ← vd_root_router (i18n 分发层)
images://vd/i18n-<locale>/<本地化目录>/        ← vd_<locale>_root_router (按业务维度分组)
images://vd/i18n-<locale>/<本地化目录>/<...>/  ← 业务 provider (vd_albums_provider 等)
```
...
VD 目录在不同语言下显示不同名（"画册" / "By Album" / "アルバム別"）。
...
// vd/vd_zh_CN_root_router.json5  (中文翻译树)
{
    ...
    "list": {
        "画册":  { "provider": "vd_albums_provider" },
...
2. 主机拼接路径前缀 `images://vd/i18n-zh_CN/<rest>` 后交给 provider runtime
...
返回对象包含 `id`、`name`、`description`、`baseUrl`，并额外提供按当前 VD locale 解析好的扁平 `displayName`；DSL 路径显示应优先消费 `displayName`，避免直接读取对象形态的 manifest name。
...
```json5
{
    ...
    "list": {
        ...
        // info.displayName 是主机按 VD locale 解析好的显示名
...
要点：
- `${composed}` 被嵌入子查询，自动继承 schema from / 上游 where / join / fields
...
```json5
{
    ...
    "list": {
        ...
        // 此处 plugin.info.displayName 是主机已经做过 i18n 翻译的显示名
        "${plugin.meta.info.displayName} - ${plugin.meta.info.plugin_id}": {
...
- 显示名 `info.displayName` 由主机的 `get_plugin` 决定——也就是说 i18n 翻译在 SQL 层完成，
  DSL 只负责消费现成结果
...
```rust
// 伪代码 — 实际在 src-tauri/kabegame-core/src/db/mod.rs 一类的初始化点
...
主机要保证：
- 函数返回的 JSON 带有按当前 locale 翻译的 `displayName`（让 DSL 拿到的就是显示名）
...
走一遍 `images://vd/i18n-zh_CN/按插件/PluginA - some.plugin.id/某画册/` 的折叠：

| 段 | 命中位置 | 累积变化 |
|---|---|---|
| `images://vd` | `images_root_provider.list.vd` → `vd_root_router` | 进入 VD 根 |
...
| `/PluginA - some.plugin.id` | `vd_plugins_router.resolve` 正则截取 plugin_id | `properties.plugin_id = "some.plugin.id"`; 进入 `vd_plugin_router`，给上游 query 加 `WHERE images.plugin_id = ?` |
...
- VD 现有 provider 文件：[../../src-tauri/kabegame-core/src/providers/dsl/vd/](../../src-tauri/kabegame-core/src/providers/dsl/vd/)
