...
收集逻辑集中在 [scripts/plugins/os-plugin.ts](../../scripts/plugins/os-plugin.ts) 的 `OSPlugin.bundleLibs()`,挂在 `beforeBuild` hook,**只在 `deno task b` build 期、main 组件、非 android/web 时触发**。Linux 的 light 模式不收 fuse；FFmpeg 编解码依赖已进入二进制，不再收集对应 `.so`。
...
| Linux | ELF `DT_RUNPATH`（主程序为 `$ORIGIN/../lib/kabegame`，helper 为 `$ORIGIN`） | `/usr/bin/kabegame`、`/usr/lib/kabegame/kabegame-cef-helper` | `/usr/lib/kabegame/lib*.so.*` |
...
`$ORIGIN` 是 ELF 动态加载器变量,指向可执行文件所在目录。主程序从 `/usr/bin` 跨到 `/usr/lib/kabegame`；helper 本身就在该目录，因此使用 `$ORIGIN`。
...
(回退:`bin/{platform}/{arch}/cef-build`,dev/check/test/build 共用同一份 —— 只有一套
构建档位 official build + PGO,曾经的 dev/prod variant 已删除)。

**编译产物不落在 `bin/`,是拷过去的。** 编译发生在仓库外的 `CEFBUILD` 工作区
(默认 `~/kabegame-cefbuild`),`exportCefRuntime()` 再 `cpSync` 到 `bin/{platform}/{arch}/cef-build`:
...
- **locale 白名单:`scripts/cef-locales.ts` 是单一来源**(en/ja/ko/zh_CN/zh_TW,`en` 必留作回退)。CEF 的 locale 是 Chromium 自身 UI 文案,与 app 的 Vue i18n 无关;自编发行版带全量 228 个共 ~52MB。
  ...
  - 改白名单后**必须重跑 `deno task build:chromium`** 才对已有 `cef-build` 生效(不重编 Chromium,只重走导出)。裁剪不影响签名:自编 framework 是 linker-signed adhoc,`Sealed Resources=none`,签名只覆盖 Mach-O。也不会触发 cef-dll-sys 的「下载官方无 H.264 CEF」回退 —— `check_archive_json` 只校验 `archive.json` 里的版本号,不看文件清单。
...
- `linuxBins`:`bin/linux/` 下所有文件相对路径组成的字符串数组(如 `["libx264.so.163", ...]`),模板用 `{{#each linuxBins}}` 遍历生成 deb files entries。
- `macosFrameworks`:CEF framework 绝对路径组成的字符串数组(如 `["<CEF_PATH>/Chromium Embedded Framework.framework"]`,build 期注入；dev/check 为 `[]`),模板用 `{{#each macosFrameworks}}` 拼出 frameworks 数组。
macOS 的 CEF helper **不经模板变量注入**:三平台统一为 exe 旁的单一扁平 `kabegame-cef-helper`(无 `.app` 变体,见 `src-tauri/tauri-runtime-cef/README.md` 的「CEF fork 与 patch」)。build 下三平台由 `tauri.conf.json` **顶层 `bins` 配置**(fork patch 0009,见 [../tauri/TAURI_CLI_FORK.md](../tauri/TAURI_CLI_FORK.md))驱动:`tauri build` 把清单逐个 `--bin` 传给 cargo(不再 `--bins` 全量编译,Cargo examples 不进 release),helper 随主编译一并产出;macOS 模板把 release 产物映射到 `Contents/MacOS/`,Linux 映射到 `/usr/lib/kabegame/`,**Windows 则由 NSIS 原生把 `bins` 里的辅助 bin 装到 `$INSTDIR` 根(主 exe 旁)并随卸载删除**——不再预构建、不再 stage 进 `resources/bin`、installer hook 不再搬运 helper。dev 仍由 ComponentPlugin `beforeBuild` 预构建(`tauri dev` 走 `cargo run`,无法同调用多编 bin)。

`tauri.conf.json.handlebars` 用 `{{#each linuxBins}}` 遍历注入 files entries,渲染出的 `tauri.conf.json`:
...
"macOS": {
  ...
  "frameworks": ["<repo>/bin/macos/arm64/cef-build/Chromium Embedded Framework.framework"],
  "files": {
...
- **升级 FFmpeg/x264 主版本**:重新运行 `deno task build:ffmpeg`；下次 Rust 构建会重新读取 `libav*.pc` 并静态链接更新后的库。
- **系统 x264 版本变化**:重新运行 `deno task build:ffmpeg` 后重建即可，不需要收集 SONAME 动态库。
...
| 文件 | 角色 |
|---|---|
| `scripts/plugins/os-plugin.ts` | 主入口 `bundleLibs` + 平台收集 + `verifyFFmpegBuildArtifacts` + macOS helper 运行前检查 |
...
| `scripts/build-chromium.ts` | 编 CEF/Chromium(仓库外 CEFBUILD 工作区)+ `exportCefRuntime()` 拷到 `cef-build`,途中按 locale 白名单裁剪 |
| `scripts/cef-locales.ts` | CEF locale 白名单单一来源(pak / lproj 两种排布),导出期与打包期共用 |
| `scripts/paths.ts` / `scripts/utils.ts` | 路径命名公式(`repoBuildDir`,CEF 走 `repoBuildDir("cef")`)与通用工具;Windows DLL 复制函数已迁到 os-plugin |
...
| `src-tauri/kabegame/nsis/installer-hooks.nsh` | Windows 安装期把 resources/bin DLL 与 resources/cef CEF runtime 搬到 $INSTDIR;卸载期清理;不自动安装 Dokan 驱动;helper 不经此搬运(顶层 bins 由 NSIS 原生安装) |
