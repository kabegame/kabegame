...
macOS 的 CEF helper **不经模板变量注入**:三平台统一为 exe 旁的单一扁平 `kabegame-cef-helper`(无 `.app` 变体,见 `src-tauri/tauri-runtime-cef/README.md` 的「CEF fork 与 patch」)。ComponentPlugin 的 `beforeBuild` 先构建该 bin，模板再将 release 产物映射到 `Contents/MacOS/`。
...
| `scripts/plugins/component-plugin.ts` | 渲染 tauri.conf 前注入 `linuxBins` / `macosFrameworks` |
