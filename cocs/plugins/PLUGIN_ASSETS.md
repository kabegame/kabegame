...
（更新日志），路径归一化的唯一裁决者是谁，以及这些图片除了渲染在文档正文里之外，
还被「源」页面的 hover 快捷预览当作走马灯示例图使用。
...
| 关注点 | 处理 |
|---|---|
| 顺序 | `Object.keys(assets).sort()`，与 PathQL provider `plugin://{id}/asset` 的 list 排序口径一致（字典序，非作者声明序） |
| 图注 | 插件没有提供图注元数据，用 `humanizeAssetLabel(key)` 从文件名推导（去目录与扩展名、`-`/`_` 转空格），不编造内容 |
