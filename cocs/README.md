...
- [gallery/PROVIDER_IMAGEQUERY_COMPOSABLE.md](gallery/PROVIDER_IMAGEQUERY_COMPOSABLE.md)
  - 主题：Gallery/VD 共用的 Provider + ImageQuery 可组合查询系统（**当前 Rust 实现**；未来由 DSL 替代，参见 provider-dsl/）。含**前端查询行**：画廊 / 画册详情 / 任务详情 / 畅游详情共用的 `GalleryQueryBar`——前端状态统一为**唯一查询对象 `GalleryQuery`**（搜索是原子的一个维度；查询条以简单 chip + 追加高级 chip 叠加，单分支组保留持久化边界；`no-album` 与 `hide` 并列为随行路由上下文），唯一 `navigate` 出口、`providerContextPrefix` 与 `contextBase` 的分工；以及高级查询在 detail 路由上的落地前提（`~` 组合器由引擎拦截，搜索格需三个 detail provider 的 resolve 列出 `search`）。
...
- [build/MACOS_CROSS_BUILD.md](build/MACOS_CROSS_BUILD.md)
  - 主题：**macOS 跨架构构建**（Apple Silicon ↔ Intel）。涵盖统一入口 `--target x86_64|arm64`（仅 macOS，`build`/`check`/`start` 可用，`dev` 与 android/web 拒绝）、**`scripts/paths.ts`（零 npm 依赖）作为架构解析与路径命名公式的单一来源**（`TARGET_ARCH`/`TARGET_TRIPLE`/`ARTIFACT_DIR`/`repoBuildDir()`/`CHROMIUM_DIR`，由 utils.ts re-export，回写 `KB_TARGET_ARCH`/`KB_ARTIFACT_DIR` 供子进程继承）、按架构（而非宿主）决定的目录隔离约定（两架构对称落 `bin/macos/{arm64,x86_64}/`，不再有 `-x64` 后缀）、出 Intel 包的完整命令顺序，以及踩坑：**官方 CEF 预编译包因缺 flat-subprocess patch 不可用**、**PATH 上的 Android NDK toolchain 劫持 `clang`/`ld`（故一律用 `cc` shim）**、`--target arm64` 也会换产物目录、x86_64 的 x264 硬依赖 nasm、**跨编专属 env（`PKG_CONFIG_ALLOW_CROSS` + bindgen `-target`，只有实跑 cargo 才暴露）**、**CEF 自编 `--x64-build` 不足需 `CEF_ENABLE_*`+`GN_OUT_CONFIGS` 让 gn_args 真的生成 x64 配置、并按目标架构补下 PGO profile**。
...
- [build/LINUX_BUILD_WORKFLOW.md](build/LINUX_BUILD_WORKFLOW.md)
  - 主题：**本机 Linux 构建工作流**——因本机 glibc 2.43 太新，出货用的 `.deb` 在 Ubuntu 22.04 guest（glibc 2.35）里做最终链接（不重编 CEF/Chromium/V8）。核心是 **profile 隔离 + 构建属地**模型：guest 完整挂载仓库、等同一台开发机（无 `target-22`、无 `.vm/cef-prod` 侧拷贝、无 `-22`/`-web` 后缀）；host 只跑 debug（`target/debug`），**release 与 FFmpeg/x264 产物构建只在 guest**（`target/release`），由 `requireUbuntu2204()` 守卫强制（逃生阀 `KB_ALLOW_HOST_BUILD=1`），chromium/v8 构建豁免。涵盖 virtiofs 同路径挂载、`paths.ts` 的路径/target 单一来源、glibc 地板验证；以及踩坑：**预编译 `.a` 的 `__isoc23_*`（2.38 新名符号、无版本标签、易漏判）必须重编 x264+FFmpeg**、`.pc` 烧死的绝对路径、构建脚本写死 `target/release`、`package-plugin.ts` 误跑 host cli、web docker 构建会覆盖树内 Linux FFmpeg 产物。
  - 适用场景：出货 Linux deb（标准流程）；判断某个预编译 `.a`（x264/FFmpeg/v8）能否跨 glibc 复用；排查 `version 'GLIBC_2.xx' not found`、`undefined symbol: __isoc23_*`、`.pc` prefix 失效、构建被守卫拒绝、`CARGO_TARGET_DIR` 未被某脚本尊重；复现/维护 22.04 构建 guest。
