...
Linux 的 CEF runtime 使用默认开源编解码器，不包含 H.264/AAC。因此 Linux 上的视频兼容副本统一为 **VP9/Opus WebM**（原视频存在音轨时必须保留为 Opus），视频预览缩略图统一为 **VP9 WebM**；Windows/macOS 分别维持 H.264/AAC MP4 与 H.264 MP4。原始视频始终保留不变，前端优先使用 `compatible_path` 播放。
...
| 构建目标 | 视频实现 |
|---------|---------|
| Linux standard/light desktop | rsmpeg/FFmpeg；兼容副本为 VP9/Opus WebM，视频缩略图为 VP9 WebM |
...
| 文件 | 作用 |
|------|------|
| `src-tauri/kabegame-core/src/crawler/downloader/compress.rs` | 视频预览生成与浏览器兼容副本。Linux 兼容副本输出 VP9/Opus WebM、视频预览输出 VP9 WebM；Windows/macOS 分别输出 H.264/AAC MP4 与 H.264 MP4。Android `compress_video_for_preview(&str)` 接收 content URI，走 provider/GIF 替代实现。 |
...
1. 新增视频处理调用点按平台分流：Android 用 content URI/provider；非 Android 可直接调用 rsmpeg/FFmpeg 实现。Linux 的浏览器兼容副本必须使用 VP9/Opus WebM，视频缩略图必须使用 VP9 WebM，不得生成 CEF 无法解码的 H.264/AAC MP4。
2. Linux 的“补充兼容格式”会重建所有视频；不要仅根据已有 `.webm` 路径跳过，因为旧 VP9 WebM 可能缺少 Opus 音轨。
...
- `bun run build:ffmpeg` → 在 `third/FFmpeg-build/install/` 生成 libav* 静态库（Unix）或 DLL+导入库（Windows）。Linux 另需系统 `libvpx-dev` 与 `libopus-dev`，以编入 VP9/Opus 编码器。
- `FFMPEG_PKG_CONFIG_PATH` 与 Linux 的 `FFMPEG_LINK_MODE=static` 由 `mode-plugin.ts prepareEnv` 注入；Linux 会显式静态链接 FFmpeg、`libvpx`、`libopus`，并且不编入未使用的 `libx264` 编码器；其余系统集成库保持动态链接。
