...
视频能力不再由 Cargo feature 门控：standard、kabegame-cli 桌面构建都支持新视频下载/导入；Android 支持 content URI 视频摄入但不链接 FFmpeg。
...
| 构建目标 | 视频实现 |
|---------|---------|
| Linux standard desktop | rsmpeg/FFmpeg；兼容副本为 H.264/AAC MP4，视频缩略图为 H.264 MP4 |
| Windows/macOS standard desktop | rsmpeg/FFmpeg；兼容副本为 H.264/AAC MP4，视频缩略图为 H.264 MP4 |
...
| `scripts/plugins/mode-plugin.ts` | 桌面 standard 注入 `FFMPEG_PKG_CONFIG_PATH` 并在 Windows 复制 FFmpeg DLL；Android 不注入 FFmpeg 环境。 |
