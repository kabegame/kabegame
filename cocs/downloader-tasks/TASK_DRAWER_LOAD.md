...
| 层级 | 文件路径 | 作用 |
|------|----------|------|
| 后端存储 | `src-tauri/core/src/storage/tasks.rs` | `get_tasks_page(limit, offset)`，LIMIT/OFFSET 分页，返回 `(Vec<TaskInfo>, u64)` |
| 命令 | `src-tauri/app-main/src/commands/task.rs` | `get_tasks_page` 命令，返回 `{ tasks, total }` |
| 前端 store | `packages/core/src/stores/crawler.ts` | `loadTasksPage`、`tasksTotal`、`loadTasks`（Android 全量） |
| 抽屉容器 | `apps/main/src/components/TaskDrawer.vue` | onMounted 调用 `loadTasksPage(20, 0)`；清除完成后重置为第一页 |
