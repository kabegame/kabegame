...
class KabegameWallpaperService : WallpaperService() {
    ...
    inner class KabegameWallpaperEngine : Engine() {
        ...
        private fun drawFill(canvas: Canvas, bitmap: Bitmap, paint: Paint?) {
            val scale = max(screenWidth.toFloat() / bitmap.width, screenHeight.toFloat() / bitmap.height)
        ...
        private fun drawFit(canvas: Canvas, bitmap: Bitmap, paint: Paint?) {
            val scale = min(screenWidth.toFloat() / bitmap.width, screenHeight.toFloat() / bitmap.height)
            val scaledWidth = (bitmap.width * scale).toInt()
            val scaledHeight = (bitmap.height * scale).toInt()
            val dx = (screenWidth - scaledWidth) / 2
            val dy = (screenHeight - scaledHeight) / 2
        ...
        private fun drawParallaxWallpaper(canvas: Canvas, bitmap: Bitmap) {
            ...
            val srcRect = Rect(safeSrcX, safeSrcY, safeSrcX + screenWidth, safeSrcY + screenHeight)
            ...
            when (transitionType) {
                ...
                "zoom" -> {
                    ...
                    val matrix = Matrix().apply {
                        ...
                        postTranslate(screenWidth * (1 - scale) / 2, screenHeight * (1 - scale) / 2)
                    }
                    val paint = Paint().apply { alpha = (transitionProgress * 255).toInt() }
                    canvas.save()
            ...
            val updateRunnable = object : Runnable {
                override fun run() {
                    ...
                    if (transitionProgress < 1f) {
                        transitionHandler.postDelayed(this, 16)
                    } else {
                        currentBitmap = nextBitmap
                        nextBitmap = null
                        isTransitioning = false
