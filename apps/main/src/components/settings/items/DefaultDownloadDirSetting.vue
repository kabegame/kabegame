<template>
    <div class="default-download-dir-setting">
        ...
        <div class="hint">
            生效路径：
            <el-button text size="small" class="path-button" :disabled="!effectiveDownloadDir"
                @click="handleOpenEffective">
                <el-icon>
                    <FolderOpened />
                </el-icon>
                <span class="path-text">{{ effectiveDownloadDir || "（未知）" }}</span>
            </el-button>
            <el-button v-if="(settingsStore.values.defaultDownloadDir as any)" link type="warning" :disabled="saving"
                @click="handleClear">
                恢复默认
            </el-button>
        </div>
