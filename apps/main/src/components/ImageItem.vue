...
import { ImageClickAction } from "@kabegame/core/src/stores/settings";
...
// 使用 Tauri asset protocol 重新生成可加载 URL（避免 blob url 的 GC/失效问题）
async function recreateAssetUrl(localPath: string): Promise<{ url: string; blob: Blob | null }> {
  const raw = (localPath || "").trim();
  if (!raw) return { url: "", blob: null };
  try {
    const normalizedPath = raw.trimStart().replace(/^\\\\\?\\/, "").trim();
    if (!normalizedPath) return { url: "", blob: null };
    if (!isTauri()) return { url: "", blob: null };
    const u = convertFileSrc(normalizedPath);
    const looksLikeWindowsPath = /^[a-zA-Z]:\\/.test(u) || /^[a-zA-Z]:\//.test(u);
    if (!u || looksLikeWindowsPath) return { url: "", blob: null };
    return { url: u, blob: null };
  } catch (error) {
    console.error("重新创建 asset URL 失败:", error, raw);
