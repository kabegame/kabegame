...
| `cefUserAgent` | `cefUserAgent(): string \| null` <Badge text="4.4.1" variant="note" /> | ✅ | ❌ | 返回桌面**畅游**（CEF）的默认 User-Agent；Android 返回 `null`。Cloudflare `cf_clearance` 等 cookie 绑定签发时的 UA，配合 `requireCookie()` 时应一并 `setHeader("User-Agent", ...)`。Chrome 大版本号由应用写死维护，可能落后于实际 CEF。 |
