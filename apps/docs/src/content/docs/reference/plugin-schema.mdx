...
| `kbDocAssets` | object | 否 | <Badge text="4.4.0" variant="note" /> | 文档资源白名单：key 是 Markdown 本地资源引用串，value 是插件根相对包内路径。字段存在时未注册引用打包报错；字段缺失时旧包回退扫描 Markdown。 |
...
所有 `kb*` 路径字段（包括 `kbDocAssets` value）统一走安全校验：非空、包根相对、禁 `..`、禁绝对路径 / 盘符 / 前导 `/`。

`kbDocAssets` key、Markdown 引用与运行时 `docResources` key 使用同一归一化语义：依次 trim、去 Markdown 标题后缀、截断 query/fragment、统一 `/`、百分号解码、排除外链、去前导 `/`、折叠路径段。结果大小写敏感，不会剥 `doc_root/`；因此 `./a.png` → `a.png`，`doc_root/a.png` 仍为 `doc_root/a.png`。允许 jpg/jpeg/png/gif/webp/bmp；显式资源单文件超过 2 MB 打包失败，总量超过 10 MB 警告。
...
```jsonc
{
  ...
  "kbDocAssets": { "./images/example.png": "doc_root/images/example.png" },
