...
脚本后端包括 **Rhai**、**V8** 与 **WebView**。JS 插件默认使用 V8 运行 `crawl.js`；只有确实需要浏览器 DOM、页面渲染或 Cookie 容器时才选择 WebView。本页聚焦 Rhai，JS 后端详见 [爬虫后端](/dev/crawler-backends/)。
...
若你的目标站点适合用 JS 实现签名、分页或数据整理，可以参考带 `crawl.js` 的插件；需要浏览器级登录态或 DOM 渲染时再选择 WebView，细节见 [爬虫后端](/dev/crawler-backends/)。
...
- [爬虫后端](/dev/crawler-backends/) — Rhai、V8 与 WebView 后端的选择与差异。
