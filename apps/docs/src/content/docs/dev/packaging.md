...
`kbAssets` 项必须存在且为 jpg/jpeg/png/gif/webp/bmp；单文件超过 2 MB 会硬报错，总体积超过 10 MB 会警告。完整归一化路径以 `banner` 开头的项是**橱窗图**（进插件卡片快捷预览走马灯），不必被 Markdown 引用，也不触发「声明但未引用」警告；轮播按它们在数组中的声明顺序展示。
