...
- **桌面**：将图片或文件夹拖入窗口，或在画廊右上角点击「开始收集」→ 选择「本地」打开导入对话框。
- **安卓**：在画廊右上角点击「开始收集」→ 选择「本地」，通过系统媒体选择器勾选要导入的图片。

### 桌面：拖入导入

把本地图片文件或包含图片的文件夹直接拖到主窗口中。可以同时拖入多个图片文件、多个文件夹，或它们的组合，应用会统一处理并显示在导入确认窗口中。
...
在 Android 上，点击「开始收集」会先弹出来源选择（本地 / 远程）。选择「本地」后进入 MediaPicker，可在「图片 / 视频 / 文件夹」入口中选择要导入的内容。
...
:::note
在 Android 上，图片和视频都会进入 PhotoSwipe 全屏预览；视频可在预览内播放。
:::
