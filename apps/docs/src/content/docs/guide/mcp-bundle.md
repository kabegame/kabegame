...
- 把请求转发到本机 `http://127.0.0.1:7490/mcp`（即 [MCP 服务](/guide/mcp/) 默认暴露的 HTTP 端点；如果修改过监听端口，需要同步修改 Bundle endpoint）。
...
| 条件 | 说明 |
|---|---|
| Kabegame 桌面版正在运行且已开启 MCP | Bundle 启动后需要连到 Kabegame 的 MCP 端点，默认是 `http://127.0.0.1:7490/mcp`；桌面版不在跑、MCP 未开启或端口不一致都会直接报 `UPSTREAM_REQUEST_FAILED`。 |
...
| 配置项 | 环境变量 | 默认值 | 说明 |
|---|---|---|---|
| Kabegame MCP endpoint | `KABEGAME_MCP_ENDPOINT` | `http://127.0.0.1:7490/mcp` | 本地 Kabegame MCP HTTP 端点 URL。若在设置中修改了 Kabegame 监听端口，需把此 endpoint 的端口同步改成一致。 |
...
**操作**：先确认桌面版已启动并已开启 MCP，参考 [MCP 服务](/guide/mcp/) 的「排障」小节核对端口。
