...
use crate::providers::vd::locale::{
    VdLocaleConfig, day_canonical_from_display, day_display_name,
    month_canonical_from_display, month_display_name,
};
...
impl Provider for VdByTimeProvider {
    ...
    fn list_children(&self, composed: &ImageQuery) -> Result<Vec<ChildEntry>, String> {
        ...
        Ok(children
            ...
            .map(|c| {
                ...
                ChildEntry::new(year.clone(), Arc::new(VdByTimeYearRouter { cfg, year }))
            })
            .collect())
    }
    ...
    fn get_child(&self, name: &str, composed: &ImageQuery) -> Option<Arc<dyn Provider>> {
        ...
        Some(Arc::new(VdByTimeYearRouter { cfg: self.cfg, year: name.to_string() }))
    }
...
impl Provider for VdByTimeYearRouter {
    ...
    fn list_children(&self, composed: &ImageQuery) -> Result<Vec<ChildEntry>, String> {
        ...
        Ok(children
            ...
            .filter_map(|c| {
                ...
                let display = month_display_name(cfg.locale, month_num);
                Some(ChildEntry::new(
                    display,
                    Arc::new(VdByTimeMonthRouter { cfg, year_month: c.name }),
                ))
            })
            .collect())
    }
    ...
    fn get_child(&self, name: &str, composed: &ImageQuery) -> Option<Arc<dyn Provider>> {
        ...
        let month_num = month_canonical_from_display(self.cfg.locale, name)?;
        ...
        Some(Arc::new(VdByTimeMonthRouter { cfg: self.cfg, year_month: ym }))
    }
...
impl Provider for VdByTimeMonthRouter {
    ...
    fn list_children(&self, composed: &ImageQuery) -> Result<Vec<ChildEntry>, String> {
        ...
        Ok(children
            ...
            .filter_map(|c| {
                ...
                let display = day_display_name(cfg.locale, day_num);
    ...
    fn get_child(&self, name: &str, composed: &ImageQuery) -> Option<Arc<dyn Provider>> {
        ...
        let day_num = day_canonical_from_display(self.cfg.locale, name)?;
