...
pub fn find_daemon_executable_basic() -> Result<PathBuf, String> {
    use crate::bin_finder::{find_binary, BinaryType};
    find_binary(BinaryType::Daemon)
}

/// 查找 daemon 可执行文件路径（完整版本，兼容 Tauri AppHandle）
///
/// 查找顺序：
/// 1. (Linux 生产环境) 从 PATH 中查找 `kabegame-daemon`
/// 2. 当前可执行文件同目录下的 `kabegame-daemon` / `kabegame-daemon.exe`
/// 注意：不查找 resources 目录
#[cfg(feature = "tauri")]
pub fn find_daemon_executable(app_handle: Option<&tauri::AppHandle>) -> Result<PathBuf, String> {
    use crate::bin_finder::find_binary_with_app_handle;
    find_binary_with_app_handle(crate::bin_finder::BinaryType::Daemon, app_handle)
}
...
pub async fn ensure_daemon_ready_basic() -> Result<PathBuf, String> {
    ...
    if is_daemon_available().await {
        ...
        return find_daemon_executable_basic();
    }
    ...
    if should_wait {
        ...
        let mut starting = STARTING.lock().unwrap();
        *starting = false;
        let daemon_path = find_daemon_executable_basic()?;
        return Err(format!(
            "等待 daemon 启动超时（10 秒）\n请检查 {} 能否正常启动",
            daemon_path.display()
        ));
    ...
    // 查找 daemon 可执行文件
    let daemon_exe = find_daemon_executable_basic()?;
...
pub async fn ensure_daemon_ready(
    ...
    if is_daemon_available().await {
        // 返回找到的 daemon 路径（用于错误提示）
        return find_daemon_executable(app_handle);
    }

    // 查找 daemon 可执行文件
    let daemon_exe = find_daemon_executable(app_handle)?;
