...
impl EventBroadcaster {
    pub fn new(_max_queue_size: usize) -> Self {
        ...
        let (sync_tx, mut sync_rx) = mpsc::unbounded_channel::<ArcDaemonEvent>();
        ...
        tokio::spawn(async move {
            while let Some(event) = sync_rx.recv().await {
                let event_with_id = {
                    ...
                    *next_id_guard += 1;
                    (id, event.clone()) // Arc 的 clone 是零拷贝的
                };
    ...
    pub async fn broadcast(&self, event: ArcDaemonEvent) {
        let _ = self.sync_tx.send(event);
    }

    pub fn broadcast_sync(&self, event: ArcDaemonEvent) {
        let _ = self.sync_tx.send(event);
    }
