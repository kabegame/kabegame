...
"#,
        ...
        // 数据库迁移：重建 tasks 表（移除历史遗留列：url/total_images/downloaded_images）
    ...
    pub fn add_task(&self, task: TaskInfo) -> Result<(), String> {
        ...
        conn.execute(
            "INSERT OR REPLACE INTO tasks (id, plugin_id, output_dir, user_config, output_album_id, status, progress, deleted_count, start_time, end_time, error)
    ...
    pub fn get_task(&self, task_id: &str) -> Result<Option<TaskInfo>, String> {
        ...
        let task = conn
            .query_row(
                "SELECT id, plugin_id, output_dir, user_config, output_album_id, status, progress, COALESCE(deleted_count, 0), start_time, end_time, error
                ...
                |row| {
                    Ok(TaskInfo {
                        ...
                        user_config: row.get::<_, Option<String>>(3)?
                            .and_then(|s| serde_json::from_str(&s).ok()),
                        output_album_id: row.get(4)?,
                        status: row.get(5)?,
                        progress: row.get(6)?,
                        deleted_count: row.get(7)?,
                        start_time: row.get::<_, Option<i64>>(8)?.map(|t| t as u64),
                        end_time: row.get::<_, Option<i64>>(9)?.map(|t| t as u64),
                        error: row.get(10)?,
                    })
                },
            )
...
fn table_has_column(conn: &Connection, table: &str, column: &str) -> bool {
    ...
    while let Ok(Some(row)) = rows.next() {
        // PRAGMA table_info: 1=column name
...
/// SQLite 不支持可靠的 DROP COLUMN（需要版本/兼容性判断），这里用“重建表”的方式移除 tasks.url。
fn migrate_rebuild_tasks_table(conn: &mut Connection) -> Result<(), String> {
    // 只在存在“历史遗留列”时重建（避免每次启动都重建）
