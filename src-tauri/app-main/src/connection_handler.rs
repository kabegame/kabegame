    ...
    Fut: std::future::Future<Output = CliIpcResponse> + Send,
{
    ...
    let mut event_rx: Option<
        tokio::sync::mpsc::UnboundedReceiver<(u64, kabegame_core::ipc::events::DaemonEvent)>,
    > = None;
            ...
            read_result = read_one_frame(&mut read_half) => {
                match read_result {
                    Ok(payload) => {
                        ...
                        let (req, request_id): (CliIpcRequest, Option<u64>) = match decode_frame::<kabegame_core::ipc::ipc::IpcEnvelope<CliIpcRequest>>(&payload) {
                            Ok(envelope) => {
                        ...
                        if let CliIpcRequest::SubscribeEvents { kinds } = req.clone() {
                            ...
                            if let Some(ref sm) = subscription_manager {
                                ...
                                let event_kinds: Vec<kabegame_core::ipc::events::DaemonEventKind> =
                                    if kinds.is_empty() {
                                        kabegame_core::ipc::events::DaemonEventKind::ALL.to_vec()
                                    } else {
                                        kinds
                                        .iter()
                                        .filter_map(|s| {
                                            kabegame_core::ipc::events::DaemonEventKind::from_str(s)
                                        })
                                        .collect()
                                    };
