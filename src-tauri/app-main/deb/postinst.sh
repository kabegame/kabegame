...
if [ "$1" = "configure" ]; then
  ...
  if [ -f "$DAEMON_SERVICE_SRC" ]; then
    ...
    # Enable service (but don't start automatically, let user control it)
    if command -v systemctl >/dev/null 2>&1; then
      systemctl enable kabegame-daemon.service || true
      echo "Daemon 服务已启用（默认未启动）"
      echo "要启动服务，请运行: sudo systemctl start kabegame-daemon"
    fi
  fi
  ...
  # Plasma wallpaper plugin is automatically installed by the package
  # Check if it was installed and provide information
  if [ -d "/usr/share/plasma/wallpapers/org.kabegame.wallpaper" ]; then
    echo "Plasma 壁纸插件已安装"
    echo "要使用插件，请："
  ...
  echo ""
  echo "Daemon 服务已安装并启用。要启动服务，请运行："
  echo "  sudo systemctl start kabegame-daemon"
  echo ""
  echo "要查看服务状态，请运行："
