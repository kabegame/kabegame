//! Album 相关命令。Tauri 薄包装：实现在 `commands::album`，与 Web 模式 RPC 共享。
//!
...
pub fn delete_album(album_id: String) -> Result<Value, String> {
    commands::album::delete_album(album_id)
}
...
pub fn move_album<R: Runtime>(
    ...
    commands::album::move_album(album_id, new_parent_id)
}
...
pub async fn get_album_preview(album_id: String, limit: usize) -> Result<Value, String> {
    commands::album::get_album_preview(album_id, limit)
}
...
pub async fn update_album_images_order(
    ...
    commands::album::update_album_images_order(album_id, image_orders)
}
...
pub async fn add_local_folder_album(
    ...
    commands::album::add_local_folder_album(name, parent_id, sync_folder, recursive).await
}
