//! Image 相关命令。Tauri 薄包装：实现在 `commands::image`，与 Web 模式 RPC 共享。
//!
...
pub async fn pathql_entry(path: String) -> Result<Value, String> {
    commands::image::pathql_entry(path).await
}

#[tauri::command]
pub async fn pathql_list(path: String, with_count: bool) -> Result<Value, String> {
    commands::image::pathql_list(path, with_count).await
}
...
pub async fn get_image_metadata_full(image_id: String) -> Result<Value, String> {
    commands::image::get_image_metadata_full(image_id)
}
...
pub async fn toggle_image_favorite<R: Runtime>(
    ...
    commands::image::toggle_image_favorite(image_id, favorite)
}
