...
use deno_core::{
    ...
    extension, resolve_url, serde_v8, v8, ExtensionArguments, JsRuntime, PollEventLoopOptions,
    RuntimeOptions,
};
...
const KABEGAME_V8_SNAPSHOT: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/kabegame_v8_snapshot.bin"));
// Residual lazy_loaded_js sources: entries not consumed during snapshot eval.
// Generated by build.rs from the LoadedFromFsDuringSnapshot files declared by
// deno extension crates (e.g. deno_crypto/00_crypto.js).
include!(concat!(env!("OUT_DIR"), "/residual_lazy_js.rs"));
...
impl JsPluginRuntime {
    ...
    pub fn new(ctx: KabegameOpState) -> AnyhowResult<Self> {
        ...
        // INVARIANT: this extension order must match `snapshot_extensions()` in
        // build.rs, and `kabegame_v8` must stay LAST. The snapshot bakes NO kabegame
        // ops; deno_core appends the real ops (declared below) after the snapshot's
        // op set via the external-reference prefix (see build.rs comment). Runtime
        // keeps `skip_op_registration = false` (default) so those real ops get bound
        // onto the `Deno.core.ops` object the snapshotted prelude closures captured.
        let mut runtime = JsRuntime::new(RuntimeOptions {
            module_loader: None,
            startup_snapshot: Some(KABEGAME_V8_SNAPSHOT),
            extensions: vec![
            ...
            residual_lazy_js_sources: KABEGAME_RESIDUAL_LAZY_JS,
            ..Default::default()
        });
        runtime.lazy_init_extensions(runtime_extension_args(blob_store, fetch_options, ctx))?;
        // deno_crypto creates cppgc objects (Crypto/SubtleCrypto/CryptoKey) which
        // cannot be serialized into a V8 startup snapshot. Load the module now that
        // the runtime isolate has its CppHeap available.
...
fn runtime_extension_args(
    blob_store: Arc<dyn deno_web::BlobStoreTrait>,
    fetch_options: deno_fetch::Options,
    ctx: KabegameOpState,
