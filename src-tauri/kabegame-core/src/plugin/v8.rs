...
// Residual lazy_loaded_js sources: the full set of deno extension JS that the
// prelude pulls via `Deno.core.loadExtScript`. Generated by build.rs. There is no
// V8 startup snapshot (see build.rs), so every lazy source must be inlined here.
...
impl JsPluginRuntime {
    ...
    pub fn new(ctx: KabegameOpState) -> AnyhowResult<Self> {
        ...
        // No V8 startup snapshot (see build.rs). Extensions are initialized eagerly
        // with `init(...)`, which evaluates their ESM — including the kabegame_v8
        // prelude — during `JsRuntime::new`. Every lazy source the prelude pulls via
        // loadExtScript is inlined through `residual_lazy_js_sources`. `kabegame_v8`
        // stays LAST so deno_web/deno_crypto are registered before the prelude runs.
        // Networking is host-side (op_kabegame_fetch), so there is no deno_fetch:
        // Headers/Response are implemented in prelude.js.
