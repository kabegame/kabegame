//! Android content:// URI IO 抽象，由 kabegame 通过 PickerPlugin 实现并注册。
...
/// Android content:// IO 操作抽象。由 kabegame 用 PluginHandle 实现并注册。异步 trait，避免在调度器 runtime 上 block_on 导致死锁。
#[cfg(target_os = "android")]
...
/// 注册 ContentIoProvider（仅 Android，由 kabegame 在 setup 时调用）。
#[cfg(target_os = "android")]
