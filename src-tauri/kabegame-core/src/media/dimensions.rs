...
fn avformat_media_dimensions(path: &std::path::Path) -> Option<(u32, u32)> {
    ...
    if let Some(dimensions) = crate::media::decode::tile_grid_dimensions(&fmt) {
        return Some(dimensions);
    }
    let video_idx = crate::media::decode::largest_video_stream_index(&fmt)?;
...
pub fn resolve_media_dimensions_sync(local_path: &str) -> Option<(u32, u32)> {
    let path = local_path_to_path_buf(local_path);
    if crate::media::image_type::is_video_by_path(path.as_path()) {
...
pub mod android {
    ...
    pub async fn resolve_media_dimensions(uri: &str) -> Option<(u32, u32)> {
        let mime = get_content_io_provider()
            ...
            .ok()
            .flatten();
        if crate::media::image_type::is_video_mime(&mime) {
            return resolve_video_dimensions(uri).await;
        }
        if crate::media::image_type::is_image_mime(&mime) {
            return resolve_image_dimensions(uri).await;
        }
        if crate::media::image_type::url_has_video_extension(uri) {
