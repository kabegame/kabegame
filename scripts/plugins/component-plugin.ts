  ...
  apply(bs: BuildSystem): void {
    ...
    bs.hooks.beforeBuild.tap(this.name, (comp?: string) => {
      ...
      if (existsSync(tauriConfigHandlebars)) {
        ...
        // Android：写入 applicationId 供 gen/android/app/build.gradle.kts 读取，使测试环境与生产使用不同包名
        if (component.isMain && isAndroid) {
          const appIdPropsPath = path.resolve(
            component.appDir,
            "gen",
            "android",
            "app",
            "kabegame-appid.properties",
          );
          const applicationId = bs.context.cmd!.isDev
            ? "app.kabegame.debug"
            : "app.kabegame";
          mkdirSync(path.dirname(appIdPropsPath), { recursive: true });
          writeFileSync(
            appIdPropsPath,
