...
import { copyDokan2DllToTauriReleaseDirBestEffort, stageResourceBinary } from "../build-utils";
...
export class Component {
    ...
    static components =  [this.MAIN, this.PLUGIN_EDITOR, this.DAEMON, this.CLI]
    ...
    get isDaemon() {
        return this.comp === Component.DAEMON || this.isAll
    }
    ...
    static appDir(cmp) {
        switch (cmp) {
            ...
            case this.DAEMON: {
                return path.join(SRC_TAURI_DIR, "app")
            }
            default: {
    ...
    apply(bs) {
        bs.hooks.parseParams.tap(this.name, () => {
            ...
            if (component && !component in Component.components) {
                throw new Error(`不存在的组件名称，允许的列表：${Component.components}`)
            }
            if (!component && !bs.cmd.isBuild) {
                throw new Error(`非构建模式必须用 -c 指定一个组件：${Component.components}`)
            }
            component = new Component(component);
            if (bs.context.cmd.isDev && (component.isCli || component.isDaemon)) {
                throw new Error(`当前 dev 不支持 cli 和 daemon！cli请构建后测试运行，daemon请start运行`)
            }
            if (bs.context.cmd.isStart && !(component.isDaemon || component.isCli)) {
                throw new Error(`当前 start 只支持 cli 和 daemon！`)
            }
